...
In the first version of *LPDynR* presented here, the main function uses multi-band "tiff" rasters to start the process, one per variable and with each band for one of the years of the time series. However, some functions are also included in the package to pre-read NetCDF and BIL files (i.e. *read_nc()* and *read_bil()*, respectively).
