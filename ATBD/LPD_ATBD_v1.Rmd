...
The Land Productivity Dynamics indicator is composed by two base layers (`r fig_num("f0", display = "cite")`). After the long term productivity dynamics descrived above, the socond source of information needed is the current level of land productivity. For this purpose a Local Net Scaling approach is implemented [@Prince_2009]. Such approach estimates, using Earth Observation imaginery and remote sensing tools, the level of land productivity of each pixel relative to their similar neighbours. Therefore, a first step is to derive homogeneous environmental units (i.e. Ecosystem Functional Types), which allow to estimate the potential level of productivity, or efficiency, of each pixel. 
...
The first "screening PCA" is done over the uncorrelated variables. 
