...
```{r echo=FALSE}
fig1 <- fig_num(name = "f1", caption = "Representation of the Steadiness Index for the case study")
...
The second source of information for the derivation of the Long-Term Change Map of Land Productivity is the baseline levels of the phenological or productivity variable in study, in this case Standing Biomass. 
...
After the average is calculated, *LPDynR* classifies pixels into 10 classes instead of the final three (i.e. low, medium and high) as an intermediate step. The reason for this is that, if directly opted for three, the number of pixels per category would have been classified homogeneously, and this is in contrast with what is stated by the Drylands Development Centre (www.undp.org). DDC declares that 40% of the World's land resources are drylands and, then, 40% of the pixels must be classified as "low level" of Standing Biomass. Therefore, *LPDynR* classifies the first four groups of pixels as "low", whereas the five consecutive groups as "medium" and the rest 10% of pixels with larger baseline levels, as "high".
...
Once the baseline levels of Standing Biomass have been calculated per pixel (`r fig_num("figBasLev", display = "cite")`), they are combined with the Steadiness Index, resulting 12 combined categories as seen in `r fig_num("figStead_BasLev", display = "cite")`. The following lines of code show how *LPDynR* proceeds to do it:
