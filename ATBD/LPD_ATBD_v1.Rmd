...
The first of the three metrics which integrates the Long-Term Change map represents the long-term tendency of degradation the natural system tends towards, either positive or negative. It is the "Steadiness Index" which, in turn, is based on the combination of two metrics calculated per pixel: (1) the slope derived from a linear regression of the different years of the time series and (2) the net change on the same period.

The use of a linear regression would imply to respect some strict statistical assumptions if confidence intervals and significance tests want to be representative. This, together with the fact that the LPD indicatior is qualitative, is why the Steadiness Index only keeps classes of tendency. See Ivits et al. [-@Ivits_report_2013] for further explanations on this point.
