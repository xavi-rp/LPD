...
```{r echo=FALSE}
figLocNetProd <- fig_num(name = "LocNetProd", caption = "blu blu blu")
LocNetProd <- paste0(path2saveTests, "/LocalNetProductivity_LSP.jpg")
```

The Local Net Primary Production Scaling (LNS) method [@Prince_2009] is based on the use of multi-temporal satellite data to calculate the difference between the potential and actual NPP for each pixel. It will be considered potential productivity the productivity that would have been were it not for human factors and it is estimated as the maximum value of productivity within each EFT [@Prince_2009, and references therein].

Cyclic Fraction, or the summed NDVI over the growing season, is widely used as a proxy for the estimation of the current land productivity [@Fensholt_2013], as it incorporates both natural and anthropogenic factors which define the inter-annual variability of productivity. Therefore, it represents that part of the Standing Biomass which is potentially appropiated to be used by humans and the environment [@Ivits_report_2013].  

In the case study
However, the maximum land productivity within each EFT has been established to its 90 percentile, as values per pixel higher than this threshold have been considered outliers and, thus, adjusted to it.
...
For the calculation of the final LPD indicator, these levels of local productivity have been aggregated into two categories, i.e, pixels with less than 50% of the highest annual local production (within the EFT) and pixels with more or equal to 50% of annual local production.
