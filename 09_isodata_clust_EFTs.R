...
#el problema es que quan n hi ha molts pot passar que un centroide s elimini de la taula en un cicle d abans i llavors en aquest punt no selecciona res
#i dona l error. el merge s hauria de fer de tots els que estan a prop a la vegada
      ...
      if(all(clusts2merge %in% rownames(clust_centr_ini))){
        ...
        pca_data_ini_noCentr <- pca_data_ini_noCentr[!rownames(pca_data_ini_noCentr) %in% rownames(sml_cluster2merge), ]
  ...
  if(length(big_clusters) > 0){
    ...
    for (bg_clstr in big_clusters) {
      ...
      pca_data_ini_noCentr <- pca_data_ini_noCentr[!rownames(pca_data_ini_noCentr) %in% rownames(bg_cluster2split), ]
    }
  }
...
for (i in iters) {
  ...
  #print(paste0("Wilks' Lambda for... ", i))
  #print(wilks_formula$statistic)
  #print(wilks_formula$parameter)
  #print(wilks_formula$p.value)
  conon_corr <- cancor(pca_data_ini_noCentr[, 1], pca_data_ini_noCentr[, 2:nvars])$cor
...
#  print(i)
#  load(paste0(path2tempResults, "/results_Step9_", i, ".RData"), verbose = FALSE)
#  conon_corr <- c(conon_corr, cancor(pca_data_ini_noCentr[, 1], pca_data_ini_noCentr[, 2:nvars])$cor)
#}
#wilks_clust_df$CanonicalCorrel <- conon_corr

names(wilks_clust_df) <- c("Iteration", "NumberOfClusters", "WilksLambda", "CanonicalCorrel")
save(list = "wilks_clust_df", file = paste0(path2tempResults, "/results_Step9_WilksDF", ".RData"))

wilks_clust_df_bckup <- wilks_clust_df
#wilks_clust_df <- wilks_clust_df_bckup
#wilks_clust_df$WilksLambda <- round(wilks_clust_df$WilksLambda, 5)

which.min(wilks_clust_df$WilksLambda)
min(wilks_clust_df$WilksLambda)
which.max(wilks_clust_df$CanonicalCorrel)
max(wilks_clust_df$CanonicalCorrel)
...
for (i in iters) { 
  ...
  all_data <- rbind(pca_data_ini_noCentr[, 1:5], all_data)
